...
public class CustomAchievement extends AchievementItem
{
	public CustomAchievement(String key, String title, String desc, String imgUrl, String imgUrlLocked, boolean hidden)
	{
		...
		Texture tempImg;
		TextureAtlas.AtlasRegion tempRegion;
		if (isUnlocked) {
			if (imgUrl == null) {
		...
		if (tempImg != null) {
			tempRegion = new TextureAtlas.AtlasRegion(tempImg, 0, 0, tempImg.getWidth(), tempImg.getHeight());
			ReflectionHacks.setPrivate(this, AchievementItem.class, "img", tempRegion);
		}
	}
